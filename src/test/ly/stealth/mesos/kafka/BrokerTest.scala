...
class BrokerTest extends KafkaMesosTestCase {
    ...
    def matches_stickiness {
        ...
        assertNull(broker.matches(offer(host0, resources0), new Date(0)))
        assertNull(broker.matches(offer(host1, resources0), new Date(0)))

        broker.registerStart(host0, 5)
        broker.registerStop(new Date(0))

        assertNull(broker.matches(offer(host0, resources0), new Date(0)))
        assertEquals("hostname != stickiness host", broker.matches(offer(host1, resources0), new Date(0)))

        // should still work even if sticky port unavailable
        assertNull(broker.matches(offer(host0, resources1), new Date(0)))
    }
    ...
    def getSuitablePort {
        ...
        broker.port = null
        assertEquals(100, broker.getSuitablePort(ranges("100..200")))
        assertEquals(1, broker.getSuitablePort(ranges("1..99,101..200")))
