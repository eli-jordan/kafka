...
import net.elodina.mesos.util.{Constraint, Period, Range, Repr, Strings}
...
object Broker {
    ...
    class Stickiness(_period: Period = new Period("10m")) {
        ...
        def allowsHostname(hostname: String, now: Date = new Date()): Boolean = {
            ...
            if (!withinStickyPeriod(now)) return true
            this.hostname == hostname
        }

        def withinStickyPeriod(now: Date = new Date()): Boolean = {
            (stopTime != null) && (now.getTime - stopTime.getTime < period.ms)
        }
        ...
        def fromJson(node: Map[String, Object]): Unit = {
            ...
            if (node.contains("port")) port = node("port").asInstanceOf[Integer]
        }
